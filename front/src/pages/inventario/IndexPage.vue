<template>
  <q-page class="q-pa-sm">
    <div class="row items-center q-mb-sm">
      <div><div class="text-subtitle1 text-weight-bold">Inventario</div><div class="text-caption text-grey-7">Revisiones del stock físico de la tienda</div></div>
    ...
    <q-card flat bordered>
      ...
      <q-table flat dense :rows="rows" :columns="columns" row-key="id" :loading="loading" v-model:pagination="pagination" :rows-per-page-options="[20,50,100]" @request="onRequest">
        ...
        <template #body-cell-actions="p"><q-td :props="p">
          <q-btn v-if="p.row.estado==='BORRADOR'&&can('Editar Almacenes')" dense flat round color="primary" icon="fact_check" :to="`/inventario/${p.row.id}`"><q-tooltip>Llenar la revisión</q-tooltip></q-btn>
          <q-btn dense flat round color="blue-grey" icon="insights" :to="`/inventario/${p.row.id}/avance`"><q-tooltip>Ver avance y actualizar productos</q-tooltip></q-btn>
...
async function load(){
  ...
  }catch(e){proxy.$alert.error(e.response?.data?.message||'No se pudieron cargar las revisiones de inventario')}
